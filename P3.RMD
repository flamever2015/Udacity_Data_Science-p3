...
#### 1. 借款金额会随着借款人年龄、不同初始评级的条件下 变化情况
...
- 初始评级为E、F类用户，借款数据量很少。
- 从散点图分布情况来看，借款金额和年龄的函数关系可能是 y ~ (x-a)^2。

借款金额会随着借款人年龄、不同初始评级的条件下 变化情况 
- facet wrap by 性别
...
#### 4. 借款金额会随着借款人身份认证指数、不同初始评级的条件下 变化情况
...
#### 5. 借款金额会随着借款人历史成功单次借款金额（hs1LA）、不同初始评级的条件下 变化情况
...
```{r echo=FALSE}
# 创建变量 更新单次借款金额
LC_clean$更新单次借款金额 <- NA
LC_clean$更新单次借款金额 <- (LC_clean$借款金额 + LC_clean$历史成功借款金额)/(LC_clean$历史成功借款次数 + 1)

# 借款类型箱形图    
ggplot(aes(x = LC_clean$借款类型, y = LC_clean$更新单次借款金额), data = LC_clean) +
    geom_boxplot() +
    xlab("借款类型") +  
    ylab("更新单次借款金额") +
    coord_cartesian(ylim = c(0, 200000)) +
    theme(text = element_text(family = "STHeiti"))
# 统计量
by(LC_clean$更新单次借款金额, LC_clean$借款类型, summary)
```

```{r echo=FALSE}
# 初始评级箱形图
ggplot(aes(x = LC_clean$初始评级, y = LC_clean$更新单次借款金额), data = LC_clean) +
    geom_boxplot() +
    xlab("初始评级") +  
    ylab("单次还款金额") +
...
# 统计量
by(LC_clean$更新单次借款金额, LC_clean$初始评级, summary)
```
...
```{r echo=FALSE}
m1 <- lm(I(log(LC_clean$借款金额)) ~  I(abs(LC_clean$年龄 - 29)^2), data = LC_clean)
